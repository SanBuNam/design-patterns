...
namespace GOF_design_patterns.BehavioralPatterns
{
    ...
    Concept
    This pattern deals with evaluating sentences in a language. You need to define a
    grammar to represent the language, and then the interpreter will deal with that grammar.
    For example, in this example, you will transform a three-digit integer that is input into its
    equivalent word form (in other words, a string).
    Real-Life Example
    You can think of a translator who translates a foreign language. You can also consider
    music notes as a grammar, where musicians play the role of interpreters.
    Computer World Example
    The Java compiler interprets the Java source code into bytecode that is understandable
    by the Java Virtual Machine. In C#, the source code is converted to MSIL intermediate
    code, which is interpreted by the CLR. Upon execution, this MSIL is converted to native
    code (binary executable code) by the JIT compiler
     */
